...
def verif_fichiers():
    ...
    #vérification de la présence du fichier texte et du fichier de référence
    if csv.exists():
        pass
    else:
...
def verif_fournisseurs():
    #vérification des fournisseurs du fichier texte, si nouveau qui n'est pas dans le fichier de référence
    # possibilité de l'ajouter.
    champs = []
...
def demande_fournisseur(fenetre):
    #fenêtre intermédiaire pour l'ajout d'un fournisseur
...
def creation_fournisseur():
    #fenêtre permettant d'ajouter le fournisseur en rentrant les codes et nouveaux codes courts
...
def ajout_fournisseur_excel(ccourt1, nvccourt1):
    #ajout du fournisseur suivant les infos rentrées dans le fichier excel
